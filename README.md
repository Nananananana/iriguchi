...
| **perf** | **at a floor** | | **Cold start, because a consumer spawns a process per prompt.** 125 ms → 95 ms of iriguchi's own cost. Re-profiled with `-X importtime` afterwards: what remains is `dataclasses`+`inspect` (~32 ms), `typing` for the ports' `Protocol`s (~17 ms, not deferrable — `TYPE_CHECKING` is imported *from* `typing`), and modules a `route` genuinely needs. Deciding is 0.044 ms. The next round should try to disprove this with `-X importtime`, not trust it. |
