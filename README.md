...
| **perf** | ongoing | | **Cold start, because a consumer spawns a process per prompt.** 125 ms → 95 ms of iriguchi's own cost. Re-profiled with `-X importtime` afterwards: what remains is `dataclasses`+`inspect` (~32 ms), `typing` for the ports' `Protocol`s (~17 ms, not deferrable — `TYPE_CHECKING` is imported *from* `typing`), and modules a `route` genuinely needs. Deciding is 0.044 ms. A round that said *at a floor* was disproved the same day by asking what a decision costs with only the imports it needs — 64 ms against 98 ms through the CLI — so `pathlib` and the mamori scanner followed the others out: 139 → 132 modules. Deciding is 0.044 ms. Try to disprove this too. |
